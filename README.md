...
<p> All catalogs control egress interface for traffic generated inside service VPNz using ipsec preference field in the tunnel configuration of the transport. When doing active/Active, the ipsec pref is the same for both tunnels to toad balance traffic. All traffic that is DIA will use the current active underlay WAN interface. In case of Active/Active both interfaces will be used for such traffic.</p>
